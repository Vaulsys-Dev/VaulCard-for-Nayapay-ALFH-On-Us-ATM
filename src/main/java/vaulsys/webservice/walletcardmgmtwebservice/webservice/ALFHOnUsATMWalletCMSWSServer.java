...
public class ALFHOnUsATMWalletCMSWSServer implements Runnable {
    Logger logger = Logger.getLogger(this.getClass());
    public static long id=0;
    //public static ExecutorService executor;// = MessageManager.threadPool;

    public ALFHOnUsATMWalletCMSWSServer()
    {
    ...
    public void run() {
        try {
            Thread.sleep(7000);
            logger.info("Starting ALFHOnUsATMWalletCMSWSServer....!");
            ...
            connector.setPort(GlobalContext.getInstance().getChannelbyId(ChannelCodes.WALLETALFHONUSATM).getPort());
