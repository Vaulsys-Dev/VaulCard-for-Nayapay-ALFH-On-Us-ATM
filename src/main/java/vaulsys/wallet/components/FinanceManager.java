...
//            }
            ...
            //Huzaifa, Date : 6th March 2024, Tranking ID : NAP-P5-23 - Segregation of ATM On Us Channels Bank - UBL & BAFL
            if (wsmodel.getBankcode().equals("ALFH") && wsmodel.getAcqbin().equals("62710000000")) {
                settlementaccount = GetEMICollectionAccount(account.getCurrency(), AccType.CAT_BAFL_ONUS_ATM_SETT_ACCT);
                settlementWallet = GetEMIWallet(account.getCurrency(), AccType.CAT_BAFL_ONUS_ATM_SETT_WLLT);
            }
            ...
            if (NayapayChargeAmount > 0) {
                ...
                //Huzaifa, Date : 6th March 2024, Tranking ID : NAP-P5-23 - Segregation of ATM On Us Channels Bank - UBL & BAFL
                if (wsmodel.getBankcode().equals("ALFH") && wsmodel.getAcqbin().equals("62710000000")) {
                    onUsATMFeeAccount = GetEMICollectionAccount(account.getCurrency(), AccType.CAT_BAFL_ONUS_ATM_FEE_ACCT);
                    onUsATMFeeWallet = GetEMIWallet(account.getCurrency(), AccType.CAT_BAFL_ONUS_ATM_FEE_WLLT);
                }
            ...
            if (receiptCharges > 0) {
                ...
                if (wsmodel.getBankcode().equals("ALFH") && wsmodel.getAcqbin().equals("62710000000")) {
                    receiptChargesAccount = GetEMICollectionAccount(account.getCurrency(), AccType.CAT_BAFL_ONUS_ATM_RECEIPT_CHRG_ACCT);
                    receiptChargesWallet = GetEMIWallet(account.getCurrency(), AccType.CAT_BAFL_ONUS_ATM_RECEIPT_CHRG_WLLT);
                }
            ...
            if (balInqCharges > 0) {
                ...
                //Huzaifa, Date : 6th March 2024, Tranking ID : NAP-P5-23 - Segregation of ATM On Us Channels Bank - UBL & BAFL
                if (wsmodel.getBankcode().equals("ALFH") && wsmodel.getAcqbin().equals("62710000000")) {
                    BalanceInquiryAccount = GetEMICollectionAccount(account.getCurrency(), AccType.CAT_BAFL_ONUS_BALANCE_INQUIRY_CHRG_ACCT);
                    BalanceInquiryWallet = GetEMIWallet(account.getCurrency(), AccType.CAT_BAFL_ONUS_BALANCE_INQUIRY_CHRG_WLLT);
                }
...
//        balanceLog.setClosingBalance(StringUtils.leftPad(settlementaccount.getAvailableBalance(), 12, "0"));
    ...
    @Transactional
    public static boolean OnUsCardBasedCashWithdrawalReversal(WalletCMSWsEntity wsmodel, CMSAccount account) {
        try {
            logger.info("UnLoading Wallet for OnUsCardBasedCashWithdrawalReversal Transaction....");
...
//                UpdateandLogTaxAccount(wsmodel, account.getCurrency(), TxnFlag.CREDIT);
            ...
            logger.info("OnUsCardBasedCashWithdrawalReversal in Wallet completed successfully!");
            ...
            logger.error("Exception caught while Performing OnUsCardBasedCashWithdrawalReversal in wallet...");
            e.printStackTrace();
            logger.error(WebServiceUtil.getStrException(e));
            wsmodel.setRespcode(ISOResponseCodes.UNABLE_TO_PROCESS); //46 - Unable to Process ; Refer to Doc
            return false;
